...
def build_style(k=1.0):
    ...
    return f"""
    * {{ font-family: {UI_FONT}; }}
    #card       {{ background: #1c1f26; border: 1px solid #2f343f;
                  border-radius: {px(14)}px; }}
    #word       {{ color: #ffffff; font-size: {px(26)}px; font-weight: 600; }}
    #ipa        {{ color: #7fd4ff; font-size: {px(16)}px; }}
    #trans      {{ color: #ffd479; font-size: {px(18)}px; }}
    #hint       {{ color: #5b6272; font-size: {px(16)}px; font-style: italic; }}
    #example    {{ color: #c3c9d6; font-size: {px(14)}px; }}
    #example a  {{ color: #c3c9d6; text-decoration: none; }}
    #example a:hover {{ color: #ffd479; text-decoration: underline; }}
    #exampleRu  {{ color: #6f7789; font-size: {px(13)}px; }}
    #meta       {{ color: #565d6d; font-size: {px(11)}px; }}
    #formsBox   {{ background: #22262f; border: 1px solid #2f343f;
                  border-radius: {px(9)}px; }}
    #formTag    {{ color: #6b7280; font-size: {px(11)}px; font-weight: 600; }}
    #formTagV3  {{ color: #ffb454; font-size: {px(11)}px; font-weight: 600; }}
    #formWord   {{ color: #e6eaf2; font-size: {px(16)}px; font-weight: 600; }}
    #formWordV3 {{ color: #ffd479; font-size: {px(16)}px; font-weight: 600; }}
    #formIpa    {{ color: #6a93a8; font-size: {px(12)}px; }}
    #feedOk     {{ color: #3ddc91; font-size: {px(14)}px; font-weight: 600; }}
    #feedBad    {{ color: #ff6b6b; font-size: {px(14)}px; font-weight: 600; }}
    #modeTitle  {{ color: #7fd4ff; font-size: {px(11)}px; letter-spacing: 1px; }}
    ...
    QPushButton#mode   {{ background: transparent; border: 1px solid #333a47;
                  color: #79818f; padding: {px(5)}px {px(10)}px; font-size: {px(12)}px; }}
    ...
    def _linkify(self, text, current_word):
        ...
        for token in re.split(r"(\W+)", text):
            ...
            if (token.isalpha() and len(token) > 2 and low not in self.STOP_WORDS
                ...
                out.append(f'<a href="w:{low}">{token}</a>')
            else:
                out.append(token)
        return "".join(out)
